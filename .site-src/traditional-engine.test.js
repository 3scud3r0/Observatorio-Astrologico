...
}
{
  ...
  assert.equal(E.triplicityRulers(0,'day','dorothean').participating,'Saturno');
  assert.equal(E.triplicityRulers(0,'day','ptolemaic').participating,'Marte');
  assert.equal(E.triplicityRulers(95,'night','dorothean').primary,'Marte');
  assert.equal(E.triplicityRulers(95,'night','ptolemaic').primary,'Lua');
  assert.equal(E.termRuler(13,'ptolemaic').ruler,'Vênus');
  assert.equal(E.termRuler(13,'egyptian').ruler,'Mercúrio');
  assert.equal(E.essentialDignity('Saturno',31,'day').peregrine,true);
  const contextual=E.contextualDignity('Mercúrio',101,100,'day',{triplicitySchool:'dorothean',termSchool:'egyptian'});
  assert.equal(contextual.solar.state,'combust');
  assert.equal(contextual.essential.termSchool,'egyptian');
  assert.equal(E.solarCondition(100.1,100).state,'cazimi');
  assert.equal(E.solarCondition(110,100).state,'under-beams');
  assert.equal(E.solarCondition(120,100).state,'free');
  assert.throws(()=>E.solarCondition(100,100,{cazimi:9,combustion:8,underBeams:17}),/obedecer/);
  const applying=E.aspectState({name:'Marte',lon:89,speed:1},{name:'Saturno',lon:0,speed:0},90,{aspectOrb:2});
  assert.equal(applying.phase,'applying');assert.equal(applying.inOrb,true);close(applying.effectiveOrb,2);
  const separating=E.aspectState({name:'Marte',lon:91,speed:1},{name:'Saturno',lon:0,speed:0},90,{aspectOrb:2});
  assert.equal(separating.phase,'separating');
  close(E.aspectState({name:'Marte',lon:91,speed:1},{name:'Saturno',lon:0,speed:0},90,{orbPolicy:'body-moiety'}).effectiveOrb,8.25);
  assert.throws(()=>E.aspectState({name:'Marte',lon:91,speed:1},{name:'Saturno',lon:0,speed:0},90,{orbPolicy:'bad'}),/Política/);
