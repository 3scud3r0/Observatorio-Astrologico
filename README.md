...
- `.site-src/research-analysis.js` — separação entre todas as oportunidades, janelas abertas, ausência de avaliação, inconclusivos e resultados finais; taxas-base numéricas pré-registradas, intervalos descritivos e grupos por técnica sem supor independência.
- `.site-src/research-pdf.js` — relatório local paginado da hipótese selada e do histórico de avaliações; usa o gerador PDF já disponibilizado no Observatório, com backup JSON como alternativa caso a biblioteca esteja indisponível.
- `.site-src/atlas-resume.js` — retoma consultas Atlas idênticas a partir do primeiro volume ausente após validar o fingerprint do contrato e os metadados dos volumes concluídos no IndexedDB; janelas que atravessam volumes ainda exigem consolidação própria.
- `.site-src/build-metrics.py` — registra as dimensões reais do HTML e dos ativos Swiss publicados, com compressão gzip, em `build-metrics.json`. No build de 21/09/2026, o HTML inicial media 15.920.534 bytes (6.123.632 gzip), **acima da meta de 500.000 bytes**; métricas de transferência não equivalem a FCP/TTI/RAM aferidos em celular.
